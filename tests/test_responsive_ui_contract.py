...
class ResponsiveUiContractTests(unittest.TestCase):
    ...
    def test_heatmap_schema_v9_filter_zero_value_and_realtime_contract(self):
        html = (STATIC / "heatmap.html").read_text(encoding="utf-8")
        script = (STATIC / "heatmap.js").read_text(encoding="utf-8")
        backend = (ROOT / "heatmap_engine.py").read_text(encoding="utf-8")
        self.assertIn("HEATMAP_SCHEMA_VERSION = 9", backend)
        self.assertIn("filter_groups", backend)
        self.assertIn("index_memberships", backend)
        self.assertIn("visual_universe", backend)
        self.assertIn("Ngành / chỉ số", html)
        self.assertIn('<optgroup label="Chỉ số">', script)
        self.assertIn('<optgroup label="Ngành">', script)
        self.assertIn("positiveTotal / (9 * zero.length)", script)
        self.assertIn("data(root.leaves(), (d) => d.data.symbol)", script)
        self.assertNotIn("svg.selectAll('*').remove()", script)
        self.assertIn("state.loadController.abort()", script)
        self.assertIn("Chỉ mã có giao dịch", script)
    ...
    def test_market_bubbles_navigation_canvas_and_responsive_contract(self):
        ...
        self.assertIn("INDEX:VN30", script)
        self.assertIn("itemSectorNames", script)
        self.assertIn("filter_groups", script)
        self.assertIn("lựa chọn VN30 đã được tạm khóa", script)
        self.assertIn('<optgroup label="Chỉ số">', script)
        self.assertIn('<optgroup label="Ngành">', script)
        self.assertIn("return true;", script)
        self.assertNotIn(".velocityDecay(.32)", script)
        self.assertIn("live ? 5000 : 60000", script)
        self.assertIn("applyRealtimePayload(payload, previousPayload)", script)
        self.assertIn("session_reset_applied", script)
        self.assertIn("SESSION_NOT_STARTED", script)
        self.assertIn("kích thước chia đều", script)
        self.assertIn("Không có mã</option>", script)
        self.assertIn("buildNodes({ preserveCamera: true, initialAlpha: .16 })", script)
        self.assertIn("sessionLabel(phase)", script)
        self.assertNotIn(".bubble-session { display: none; }", css)
        self.assertIn("Minh chứng phép tính", script)
        self.assertIn("calculationText(node)", script)
        self.assertIn("data_confidence === 'VERIFIED'", script)
        self.assertIn("displayedChange(node)", script)
        self.assertIn("Không xác minh được biến động", script)
        self.assertIn("node.reason_message", script)
        self.assertIn("không công bố %", script)
        self.assertIn("Không xác minh được", html)
        self.assertIn("bubble-tooltip-warning", css)
